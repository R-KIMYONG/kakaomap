"use client";
import { KakaoMapPageProps } from "@/types/kakaomap.type";
...
const KakaoMapPage = ({ initialPosition }: KakaoMapPageProps) => {
  ...
  useEffect(() => {
    if (map && searchMarkers.length > 0) {
      map.panTo(
        new kakao.maps.LatLng(
          searchMarkers[0].position.lat,
          searchMarkers[0].position.lng
        )
      );
    }
  }, [map, searchMarkers]);
          ...
          >
            <ul className="grid gap-4 ">
              {searchMarkers.map((place, index) => (
                <li
                  key={place.url + index}
                  className={`border-b-gray-200 text-black`}
                >
        ...
        {markerPosition && (
          <MapMarker position={markerPosition}>
            <div className="w-[400px] p-4 bg-white rounded-lg shadow-xl text-green-800 z-50 absolute top-[-100px] left-[-10px]">
              ...
              <button
                onClick={() => alert("추가 기능은 아직 구현되지 않았습니다.")}
                className="mt-2 p-2 bg-green-500 text-white rounded-lg"
              >
                추가
              </button>
            ...
            {showMarkerInfo === index && (
              <div className="w-[400px] p-4 bg-white rounded-lg shadow-xl text-green-800 z-50 relative top-[-1px] left-[-1px]">
                ...
                <div
                  ...
                  onClick={() => setShowMarkerInfo(null)}
                >
                  X
                </div>
              </div>
